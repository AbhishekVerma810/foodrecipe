...
function Home() {
  ...
  const getRecipes = async () => {
    ...
    const typeParam = filter.type ? `&q=${filter.type}` : '';
    const response = await fetch(
      `https://api.edamam.com/search?q=${query}${foodParam}${typeParam}&app_id=${YOUR_APP_ID}&app_key=${YOUR_APP_KEY}&from=${pagination.from}&to=${pagination.to}`
    );
    const data = await response.json();
    setRecipes(data.hits);
  };
